...
class Crawl4AIScraper:
    ...
    def _parse_sam_gov_api_response(self, data: Dict, keywords: List[str]) -> List[TenderData]:
        ...
        for item in results:
            try:
                ...
                opportunity_url = f"https://sam.gov/opp/{solicitation_number}" if solicitation_number else "https://sam.gov"
    ...
    def _parse_html_content(self, html: str, keywords: List[str]) -> List[TenderData]:
        ...
        if not items:
            ...
            items = soup.find_all(text=lambda text: text and any(keyword.lower() in text.lower() for keyword in keywords))
    ...
    def _calculate_relevance(self, title: str, description: str, keywords: List[str]) -> float:
        ...
        industry_terms = ['window', 'door', 'glazing', 'fenestration', 'curtain wall', 'storefront']
